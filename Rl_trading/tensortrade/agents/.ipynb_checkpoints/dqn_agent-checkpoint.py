...
class DQNAgent(Agent):
    ...
    def get_action(self, state: np.ndarray, **kwargs) -> int:
        ...
        if rand < threshold:
            ...
            return np.argmax(self.policy_network.predict(policy_input)).reshape(-1)  ##获取action时，注意只有state作为输入，但action本身会影响这个网络，MRP转化成MDP
    ...
    def _apply_gradient_descent(self, memory: ReplayMemory, batch_size: int, learning_rate: float, discount_factor: float):
        ...
        with tf.GradientTape() as tape:
            ...
            next_state_values = tf.where(
                done_batch,
                tf.zeros(batch_size),  #by default, the next_state_value of done(terminal) state is 0(i.e., the future return at terminal is 0 since there's no further steps).
                tf.math.reduce_max(self.target_network(next_state_batch), axis=1)  #greedy policy(the paradigm is off-policy since the action we assume it will take at the next step is from any policy other than the the epsilon-policy the agent uses at the current step)
    ...
    def train(self,
        ...
        for i in tqdm(range(n_episodes)):
            ...
            while not done and (True if max_episode_timesteps is None else steps_done < max_episode_timesteps):
                ...
                self._apply_gradient_descent(self.memory, batch_size, learning_rate, discount_factor)
            ...
            if evaluate_every_n_episode is not None and episode % evaluate_every_n_episode == 0:
                ...
                while self.env.feed.has_next():
                    ...
                    steps_done += 1
                    state = next_state
                print('Evaluation after {} episodes ({} steps) with average reward: {}'.format(episode, steps_done, reward_sum/steps_done))
